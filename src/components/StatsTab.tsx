...
export function StatsTab({ state }: StatsTabProps) {
    ...
    const dailyAvg = period > 0 ? totalCarbs / period : 0;

    // Daily carbs for chart
    ...
    }, [mealHistory, selectedMember, periodStart]);
        ...
        return (
            <div className="space-y-6 pb-4">
                ...
                <div className="text-center py-12 bg-white rounded-3xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-400 text-sm">No hay miembros diabéticos configurados.</p>
                    <p className="text-gray-300 text-xs mt-1">Marca a alguien como diabético en la pestaña Familia.</p>
                </div>
            </div>
        );
    }
        ...
        <div className="space-y-5 pb-4">
                ...
                <div className="flex gap-2">
                    {diabeticMembers.map(dm => (
                        <button
                            ...
                            className={`flex-1 py-2 px-3 rounded-xl text-sm font-bold transition-colors ${selectedMemberId === dm.id ? 'bg-blue-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                        >
                            {dm.name}
                        </button>
                    ))}
            ...
            <div className="flex gap-2">
                {([7, 14, 30] as Period[]).map(p => (
                    <button
                        key={p}
                        onClick={() => setPeriod(p)}
                        className={`flex-1 py-2 rounded-xl text-xs font-bold transition-colors ${period === p ? 'bg-purple-500 text-white' : 'bg-gray-100 text-gray-500 hover:bg-gray-200'}`}
                    >
                        {p} días
                    </button>
                ))}
            ...
            <div className="flex gap-1.5">
                {(['todas', 'desayuno', 'comida', 'cena'] as MealType[]).map(type => (
                    <button
                        ...
                        className={`flex-1 py-1.5 rounded-lg text-[10px] font-bold transition-colors ${mealFilter === type ? 'bg-orange-500 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}
                    >
                        {getMealLabel(type)}
                    </button>
                ))}
            ...
            {mealsCount === 0 ? (
                <div className="text-center py-8 bg-white rounded-3xl border border-dashed border-gray-200">
                    ...
                    <p className="text-gray-300 text-xs mt-1">Guarda comidas en el historial para ver estadísticas.</p>
                </div>
            ) : (
                <>
                    {/* Summary cards */}
                    <div className="grid grid-cols-2 gap-2">
                        <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                            ...
                            <div className="text-2xl font-black text-purple-600 leading-none">{meanCarbs.toFixed(1)}<span className="text-xs font-normal ml-1">g</span></div>
                        </div>
                        <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                            <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">HC Diario Medio</div>
                            <div className="text-2xl font-black text-blue-600 leading-none">{dailyAvg.toFixed(1)}<span className="text-xs font-normal ml-1">g</span></div>
                        </div>
                        <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                            <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">Comidas</div>
                            <div className="text-2xl font-black text-gray-800 leading-none">{mealsCount}</div>
                        </div>
                        <div className="bg-white p-3 rounded-2xl shadow-sm border border-gray-100">
                            <div className="text-[10px] text-gray-400 font-bold uppercase mb-1">HC Total</div>
                            <div className="text-2xl font-black text-orange-600 leading-none">{totalCarbs.toFixed(0)}<span className="text-xs font-normal ml-1">g</span></div>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                        {maxEntry && (
                            <div className="bg-red-50 p-3 rounded-2xl border border-red-100">
                                ...
                                <div className="text-lg font-black text-red-600 leading-none">{maxEntry.carbs.toFixed(1)}g</div>
                        ...
                        {minEntry && (
                            <div className="bg-green-50 p-3 rounded-2xl border border-green-100">
                                ...
                                <div className="text-lg font-black text-green-600 leading-none">{minEntry.carbs.toFixed(1)}g</div>
                        ...
                        <div className="space-y-2">
                            {meanByMealType.map(({ type, label, mean, count }) => (
                                <div key={type} className="flex items-center gap-3">
                                    <div className="text-xs font-medium text-gray-600 w-24">{label}</div>
                                    <div className="flex-1 bg-gray-100 rounded-full h-5 overflow-hidden">
                                        <div
                                            ...
                                            style={{ width: `${Math.min(100, (mean / (maxDailyCarbs || 1)) * 100)}%` }}
                                        />
                                    </div>
                                    <div className="text-xs font-bold text-gray-600 min-w-[4ch] text-right">{mean.toFixed(0)}g</div>
                    ...
                    <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100">
                        ...
                        <div className="flex items-end gap-[2px] h-32">
                            {dailyData.map((day, i) => {
                                const height = maxDailyCarbs > 0 ? (day.carbs / maxDailyCarbs) * 100 : 0;
                                return (
                                    <div key={day.date} className="flex-1 flex flex-col items-center gap-1 group relative">
                                        <div
                                            className={`w-full rounded-t transition-all ${day.carbs > 0 ? 'bg-purple-400 group-hover:bg-purple-500' : 'bg-gray-100'}`}
                                        ...
                                        {/* Tooltip on hover */}
                                        {day.carbs > 0 && (
                                            <div className="absolute -top-8 bg-gray-900 text-white text-[9px] px-2 py-0.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap z-10">
