...
import { ChefHat, Search, Plus, Trash2, Save, ChevronRight, Utensils, ArrowLeft, Sparkles, Mic, MicOff, AlertTriangle, MessageSquare } from 'lucide-react';
...
export function RecipeTab({ state }: RecipeTabProps) {
    ...
    const suggestedIngredients = React.useMemo(() => {
        ...
        if (fallback.length === 0 && currentRecipeIngredients.length === 0) {
            ...
            const defaultPopular = ['Aceite', 'Pan', 'Leche', 'Patata', 'Arroz'];
                        ...
                        <div className="flex flex-wrap gap-3 mb-3">
                            {diabeticMembers.map(dm => {
                                const dmCarbs = activeFamilyProportionSum > 0
                                    ...
                                    : 0;
                                return (
                                    <div key={dm.id} className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 px-3 py-2 rounded-xl border border-blue-100 dark:border-blue-800">
                                        <div>
                                        ...
                                        <button
                                            onClick={() => shareDiabeticCarbs(dm, dmCarbs)}
                                            className="ml-1 p-2 bg-green-500 text-white rounded-xl hover:bg-green-600 transition-colors"
                                            title={dm.phone ? `Enviar a ${dm.name} por WhatsApp` : 'Compartir HC por WhatsApp'}
                                        >
